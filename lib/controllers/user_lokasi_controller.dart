...
class UserLokasiController extends GetxController {
  ...
  void onInit() {
    ...
    // CEK STATUS (TANPA LOADING)
    cekStatusHariIni();

    // FETCH LOKASI DI BACKGROUND (TANPA LOADING)
    fetchUserLokasi();
  }
  ...
  // ================= DETEKSI WAJAH (HANYA UNTUK MOBILE) =================
  Future<bool> _detectFace(File imageFile) async {
    // CEK APAKAH DI WEB
    if (kIsWeb) {
  ...
  // ================= AMBIL FOTO (DENGAN DETEKSI PLATFORM) =================
  Future<File?> takePhotoWithFaceDetection() async {
    // CEK APAKAH DI WEB
    if (kIsWeb) {
      _showWebNotSupportedDialog();
  ...
  Future<void> prosesAbsensi(String tipe) async {
    ...
    if (userLokasis.isEmpty) {
      await fetchUserLokasi(); // COBA FETCH ULANG
